...
/*283 assignment 3,Tingjia Zhang: */
...
int kvm_emulate_cpuid_2(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4FFFFFFC) {
		...
		printk("CPUID(0x4FFFFFFC), exits= %d, cycles spent in exit= %ld", atomic_read(&exitCounts), atomic_long_read(&timeSpend));
	} else {
